...
const LINKS = [
    {
        name: 'Facebook',
        color: '#1877F2',
        buildUrl: (url, text) => `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}`,
        icon: (
    ...
    },
    {
        name: 'X',
        color: '#000000',
        buildUrl: (url, text) => `https://x.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent(url)}`,
        icon: (
            <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
...
export default function ShareButtons({ url, title, className = '', buttonStyle }) {
    const shareUrl = url || (typeof window !== 'undefined' ? window.location.href : '');
    const shareText = title ? `Check out this score: ${title}` : 'Check out this score';
    ...
    return (
        <div className={`d-flex gap-1 align-items-center ${className}`}>
            {LINKS.map(s => (
                <button
                    key={s.name}
                    onClick={() => window.open(s.buildUrl(shareUrl, shareText), '_blank', 'noopener,noreferrer,width=600,height=500')}
