...
const shareSvg = {
    ...
    x: <svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" /></svg>,
    reddit: <svg width="11" height="11" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="11" stroke="currentColor" strokeWidth="1.2" /><circle cx="9.2" cy="11.8" r="1.3" stroke="currentColor" strokeWidth="0.8" fill="none" /><circle cx="14.8" cy="11.8" r="1.3" stroke="currentColor" strokeWidth="0.8" fill="none" /><path d="M8.5 15.5c.8.8 2 1.3 3.5 1.3s2.7-.5 3.5-1.3" stroke="currentColor" strokeWidth="0.8" strokeLinecap="round" fill="none" /><path d="M12 3.5l-1.5 4" stroke="currentColor" strokeWidth="0.8" strokeLinecap="round" fill="none" /><circle cx="16.5" cy="6" r="0.6" fill="currentColor" /><path d="M16.5 6L14 7.5" stroke="currentColor" strokeWidth="0.5" fill="none" /></svg>,
    ...
    telegram: <svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor"><path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z" /></svg>,
};

const shareButtons = [
    { name: "Facebook", color: "#1877F2", svg: "facebook" },
    { name: "X", color: "#aaa", svg: "x" },
...
export default function Content() {
    ...
    const [scoreInstrument, setScoreInstrument] = useState(null);
    ...
    const loadUserScore = async (scoreId) => {
        ...
        try {
            ...
            if (!data) {
                const snap = await getDoc(doc(db, col("scores"), scoreId));
                if (!snap.exists()) {
            ...
            const fmt = (ts) => ts ? new Date(ts).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" }) : null;
            const createdDate = fmt(data.createdAt);
            const updatedDate = fmt(data.updatedAt);
            if (createdDate && createdDate === updatedDate) {
            ...
            setScoreUser(data.username || data.userId || null);
            setScoreInstrument(data.instrument || null);
            ...
            <>
                <Container fluid>
                    <Row className="mt-3 align-items-center">
                        <Col xs="auto"><b>Created: </b>{publishDate}</Col>
                        {lastUpdated ? <Col xs="auto"><b>Last updated: </b>{lastUpdated}</Col> : <></>}
                        {scoreInstrument ? <Col xs="auto"><b>Instrument: </b>{scoreInstrument}</Col> : <></>}
                        {scoreUser ? (
                            <Col xs="auto">
                                <b>By: </b>
                                <a href={`/profile/${scoreUser.startsWith("@") ? scoreUser : "@" + scoreUser}`} className="text-info text-decoration-none">{scoreUser}</a>
                            </Col>
                        ) : <></>}
                        <Col />
